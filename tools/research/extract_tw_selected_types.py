...
PATTERNS = (
    r"(^|\.)AppUrl$",
    r"(^|\.)AppUrlData$",
    r"(^|\.)AppLocalizeSettings$",
    r"(^|\.)AppResourceSettings$",
    r"(^|\.)AppSettings$",
    r"(^|\.)AppPokkeConfig$",
    r"(^|\.)AppReqBase$",
    r"(^|\.)ReDriveMsgPackApiFactory$",
    r"(^|\.)APISigning$",
    r"(^|\.)ResourceApi$",
    r"(^|\.)ResourceRemoteUrl$",
    r"(^|\.)ResourceDownloader$",
    r"(^|\.)ResourceConfigData$",
    r"(^|\.)ResourceConfigModel$",
    r"(^|\.)ResourceGcsSignedUrlData$",
    r"(^|\.)ResourceGcsSignedUrlModel$",
    r"(^|\.)PokkeMsgPackAPIFactory$",
    r"(^|\.)PokkeMsgPackAPI`2$",
    r"(^|\.)PokkeReqContainer$",
    r"(^|\.)PokkeUserInfo$",
    r"(^|\.)LoginAPI$",
    r"(^|\.)LoginApi$",
    r"(^|\.)LoginApi\.Login$",
    r"(^|\.)LoginApi\.Login\.Request$",
    r"(^|\.)LoginApi\.Login\.Response$",
    r"(^|\.)TW_ApiHelper$",
    r"(^|\.)Title.*Login.*$",
    r"(^|\.)Login.*UseCase.*$",
    r"(^|\.)Login.*Sequence.*$",
...
def main() -> None:
    ...
    if listed.returncode:
        raise SystemExit(f"ilspy type listing failed: {listed.returncode}")

    type_names: list[str] = []
    for line in listed.stdout.splitlines():
        value = line.strip()
        value = re.sub(r"^(Class|Struct|Interface|Enum)\s+", "", value)
        if not value:
            continue
    ...
    (args.out / "selected-types.json").write_text(
        json.dumps(records, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    if not records:
        raise SystemExit("no selected Taiwan protocol types found")
