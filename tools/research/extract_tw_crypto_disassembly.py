...
ALL_TYPE_FRAGMENTS=(
 'PokkeMsgPackAPI','PokkeMsgPackAPIFactory','PokkeAPI','PHPSession','WebRequestExt','JsonProtocol','BasicAPI',
)
...
def main():
 ...
 for m in METHOD.finditer(text):
  ...
  methods.append({'rva':int(m.group(1),16),'namespace':ns,'type':tp,'name':name_match.group(1) if name_match else '', 'signature':signature})
 ...
 if not selected: raise SystemExit('no response/crypto targets')
 ...
 # Preserve only bounded type declarations for response-flow review.
 block_dir=args.out/'type-blocks'; block_dir.mkdir(exist_ok=True)
 index=[]
 for i,(start,tp) in enumerate(type_positions):
  ns_i=bisect.bisect_right(ns_starts,start)-1; ns=ns_positions[ns_i][1] if ns_i>=0 else ''
  if ns!='A2.Http' or not any(fragment in tp for fragment in ALL_TYPE_FRAGMENTS): continue
  stop=type_positions[i+1][0] if i+1<len(type_positions) else len(text)
  block=text[start:stop][:500_000]
  name=f'{safe_name(ns+"."+tp)}.cs.txt'
  (block_dir/name).write_text(block,encoding='utf-8')
  index.append({'namespace':ns,'type':tp,'file':name,'bytes':len(block.encode())})
