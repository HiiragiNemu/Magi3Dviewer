...
try {
  const modelLoadedPromise = page.waitForEvent('console', {
    predicate: message => message.text().includes('loaded successfully'),
    timeout: 180_000,
  })
  ...
  const modelLoadedMessage = await modelLoadedPromise
  ...
  await page.waitForFunction(() => {
    ...
    return progress instanceof HTMLElement
      ...
      && !document.body.classList.contains('no-target')
  ...
  const state = await page.evaluate(() => {
    ...
    const character = document.querySelector('#character-selector')
    const progress = document.querySelector('#load-progress')
    const animation = document.querySelector('#animation-selector')
    ...
    return {
      ...
      loadProgressHidden: progress instanceof HTMLElement
        ? getComputedStyle(progress).display === 'none'
        : null,
      noTarget: document.body.classList.contains('no-target'),
      animationOptionCount: animation instanceof HTMLSelectElement
  ...
  if (state.character !== '100107') {
    throw new Error(`Wrong character loaded: ${state.character}`)
  }
  if (state.stage !== 'battle-608-00-00-001') {
    throw new Error(`Wrong stage loaded: ${state.stage}`)
  }
  if (!state.loadProgressHidden || state.noTarget) {
    throw new Error('Character load did not reach the selected/finished UI state')
  }
  ...
  if (consoleErrors.length) {
    throw new Error(
      `Console errors: ${consoleErrors.map(row => row.text).join('\n')}`,
    )
  }
} finally {
  await browser.close()
